const AppState = {
    ...
    register(userData) {
        ...
        // Set initial status and login credentials for students and companies
        if (userData.role === 'student' || userData.role === 'company') {
            userData.status = 'pending'; // pending verification
            userData.loginCode = null;
            userData.assignedPassword = null;
            userData.validityExpiry = null; // date string
    ...
    login(identifier, password, role) {
        // For T&P, login with email and password
        if (role === 'tnp') {
            const user = this.tnpOfficers.find(u => u.email === identifier && u.password === password);
            if (user) {
                this.currentUser = user;
                this.saveToStorage();
                return { success: true };
            }
            const allTnpEmails = this.tnpOfficers.map(u => u.email);
            if (allTnpEmails.includes(identifier)) {
                return { success: false, message: 'Invalid password.' };
            }
            return { success: false, message: 'User not found. Please register first.' };
        }

        // For students and companies, login with assigned code and password
        const users = role === 'student' ? this.students : this.companies;
        const user = users.find(u => u.loginCode === identifier);

        if (!user) {
            return { success: false, message: 'Invalid login code.' };
        }
        if (user.assignedPassword !== password) {
            return { success: false, message: 'Invalid password.' };
        }
        ...
        if (user.isBlocked) {
            return { success: false, message: 'User is blocked. Contact T&P admin.' };
        }
        if (user.validityExpiry && new Date(user.validityExpiry) < new Date()) {
            return { success: false, message: 'Login code expired. Contact T&P admin.' };
        }
    ...
    postJob(jobData) {
        ...
        const company = this.companies.find(c => c.id === jobData.companyId);
        if (company) {
            if (!company.jobPostings) {
                company.jobPostings = [];
            }
            company.jobPostings.push(jobData.id);
        }
        this.saveToStorage();
    },
    ...
    applyJob(studentId, jobId, coverLetter) {
        ...
        if (student) {
            ...
            student.applications.push(application.id);
        }
        this.saveToStorage();
    },
            ...
            if (status === 'accepted') {
                ...
                if (job && student) {
                    ...
                    student.placement = {
                        ...
                        package: job.salary ? job.salary.totalCtc : job.stipend,
...
document.getElementById('login-form').addEventListener('submit', function(e) {
    ...
    if (!email || !password || !role) {
        showError('Please fill in all fields.', 'login-message');
        submitBtn.classList.remove('loading');
        submitBtn.disabled = false;
        return;
    }

    const loginResult = AppState.login(email, password, role);
    if (loginResult.success) {
        ...
        showSuccess('Login successful!', 'login-message');
    } else {
        showError(loginResult.message, 'login-message');
    }
    submitBtn.classList.remove('loading');
    submitBtn.disabled = false;
});
...
document.getElementById('register-form').addEventListener('submit', function(e) {
    ...
    if (!userData.name || !userData.email || !userData.phone || !userData.password || !userData.role) {
        showError('Please fill in all required fields.', 'register-message');
        submitBtn.classList.remove('loading');
        submitBtn.disabled = false;
        return;
    }
    ...
    if (!/^\+[0-9]{1,3}[0-9]{10}$/.test(userData.phone)) {
        showError('Phone number must include country code (e.g., +919876543210).', 'register-message');
        submitBtn.classList.remove('loading');
        submitBtn.disabled = false;
        return;
    }
    ...
    if (!passwordRegex.test(userData.password)) {
        showError('Password must be at least 8 characters long, including a number and a special character.', 'register-message');
        submitBtn.classList.remove('loading');
        submitBtn.disabled = false;
        return;
    }
    ...
    if (userData.role === 'student') {
        ...
        if (!userData.course) {
            showError('Please fill in all student-specific fields.', 'register-message');
            submitBtn.classList.remove('loading');
            submitBtn.disabled = false;
            return;
        }
        ...
        if (!userData.employeeId || !userData.position || !userData.department || !userData.phone) {
            showError('Please fill in all T&P Panel-specific fields.', 'register-message');
            submitBtn.classList.remove('loading');
            submitBtn.disabled = false;
            return;
        }
        ...
        if (!userData.hrId || !userData.companyName || !userData.industry || !userData.website) {
            showError('Please fill in all company-specific fields.', 'register-message');
            submitBtn.classList.remove('loading');
            submitBtn.disabled = false;
            return;
        }
...
function openVerificationModal(role, userId) {
    ...
    `;
    document.getElementById('login-code').value = user.loginCode || '';
    document.getElementById('login-password-assign').value = '';
    document.getElementById('validity-months').value = 6;
...
// Assign login code, password, and validity to user and update status
function assignCredentials(role, userId) {
    ...
    if (!loginCode || !password || isNaN(validityMonths) || validityMonths < 1) {
        alert('Please fill in all fields correctly.');
        return;
    }

    // Check for unique login code across students and companies
    const allUsers = [...AppState.students, ...AppState.companies];
    if (allUsers.some(u => u.loginCode === loginCode && u.id !== userId)) {
        alert('Login code must be unique.');
        return;
    }
    ...
    userList[userIndex].loginCode = loginCode;
    userList[userIndex].assignedPassword = password;
    userList[userIndex].validityExpiry = expiryDate.toISOString();
    userList[userIndex].status = 'verified';
    userList[userIndex].isBlocked = false;

    AppState.saveToStorage();
    closeModal('verification-modal');

    // Show assigned credentials modal
    showAssignedCredentials(loginCode, password, expiryDate);
    // Refresh T&P dashboard lists
...
function showJobForm() {
    ...
    toggleOpportunityType(); // Call to reset required fields and display
    jobRequiredSkills = [];
    renderJobSkills();
